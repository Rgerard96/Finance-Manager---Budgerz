...
export default function MenuBar() {
  ...
  return (
    <div
      className={`bg-gray-100 rounded-lg p-5 mt-5 sm:mt-0 border border-gray-400 sm:flex-col items-center sm:items-start justify-around sm:justify-start sm:space-y-10 fixed left-5 bottom-5 standalone:bottom-6 right-5 sm:static ${
        router.pathname === '/inloggen' ? 'hidden' : router.pathname === '/registreren' ? 'hidden' : 'flex'
      }`}
    >
      <Link href='/'>
        <div className='cursor-pointer flex space-x-3 items-center text-gray-400 group'>
          <HomeIcon className='w-6 text-indigo-500 ' />
          <span className='hidden sm:block group-hover:text-indigo-500'>
            Home
          </span>
        </div>
      </Link>
